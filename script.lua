...
local PlaybackMode="Dummy" local EBSubTab="Parkour" local ShowLoja,ShowJJs,ShowEBDelta
...
local delta=corrected-rootPart.Position pcall(function() if delta.Magnitude>.02 then humanoid:Move(delta.Unit,false) else humanoid:Move(Vector3.zero,false) end end)
...
if PlaybackMode=="Direto" then local frames=Playback.Route humanoid.WalkSpeed=CONFIG.DiretoWalkSpeed
for idx=1,#frames do if not Playback.Running or Playback.WalkingToStart then break end if not RefreshCharacter()then StopPlayback("error");break end
local f=frames[idx] local pos=GetPosition(f) if not pos then break end local dest=pos+Vector3.new(0,CONFIG.GroundOffset,0) humanoid:MoveTo(dest)
local waitStart=os.clock() while Playback.Running and not Playback.WalkingToStart and RefreshCharacter() and(rootPart.Position-dest).Magnitude>2.4 do
if os.clock()-waitStart>6 then break end task.wait() end if not Playback.Running or Playback.WalkingToStart then break end
if f.j then humanoid.Jump=true pcall(function()humanoid:ChangeState(Enum.HumanoidStateType.Jumping)end) end end
if Playback.Running and not Playback.WalkingToStart then StopPlayback("completed") end return end
...
local function ShowParkours() local tf=_I("Frame") tf.Size=_U2(1,0,0,48) tf.BackgroundColor3=_K.Card tf.BorderSizePixel=0
...
expand.ZIndex=6 expand.Parent=row local ec=_I("Frame") ec.Size=_U2(1,0,0,0) ec.AutomaticSize=expandedParkour[num]and Enum.AutomaticSize.Y or Enum.AutomaticSize.None
...
Notify("ROTA SELECIONADA","Parkour "..num.." • "..cat,"Success") ShowEBDelta() end) end start.MouseButton1Click:Connect(function()
if Playback.Running and Playback.Parkour==num and not Playback.Tower then StopPlayback("cancelled") ShowEBDelta() return end local cat=selectedCategory[num]or "Lento"
...
StartPlayback(route,cat,num) task.wait(.1) ShowEBDelta() end) expand.MouseButton1Click:Connect(function() expandedParkour[num]=not expandedParkour[num] ShowEBDelta()
end) end Content.CanvasPosition=_V2() end local function ShowTowers() local tf=_I("Frame") tf.Size=_U2(1,0,0,48)
...
StopPlayback("cancelled") ShowEBDelta() return end local route=towerRoutes["Torre 1"]["Única"] if not route then
Notify("ROTA NÃO CARREGADA","A rota da Torre 1 não foi encontrada.","Error") return end StartPlayback(route,nil,nil,"Torre 1","Única") task.wait(.1) ShowEBDelta() end)
...
ex.Font=_GB ex.AutoButtonColor=false ex.Parent=r2 local tc=_I("Frame") tc.Size=_U2(1,0,0,0) tc.AutomaticSize=expandedTower2 and Enum.AutomaticSize.Y or Enum.AutomaticSize.None
...
Notify("ROTA NÃO CARREGADA","Torre 2 • "..rn,"Error") return end selectedTower2Route=rn Notify("ROTA SELECIONADA","Torre 2 • "..rn,"Success") ShowEBDelta() end) end
b2.MouseButton1Click:Connect(function() if Playback.Running and Playback.Tower=="Torre 2" then StopPlayback("cancelled") ShowEBDelta() return end
...
StartPlayback(route,nil,nil,"Torre 2",rn) task.wait(.1) ShowEBDelta() end) ex.MouseButton1Click:Connect(function() expandedTower2=not expandedTower2 ShowEBDelta() end)
Content.CanvasPosition=_V2() end
local function EBHotbarBtn(txt,sub) local b=_I("TextButton") b.Size=_UO(0,32) b.AutomaticSize=Enum.AutomaticSize.X
b.BackgroundColor3=EBSubTab==sub and _K.SelectionRedDark or _K.Panel b.BorderSizePixel=0 b.Text="  "..txt.."  " b.TextColor3=EBSubTab==sub and _K.SelectionRed or _K.Gray
b.TextSize=10 b.Font=_GB b.AutoButtonColor=false Corner(b,7) Stroke(b,EBSubTab==sub and _K.SelectionRed or _K.Stroke,EBSubTab==sub and 1.5 or 1) return b end
local function ShowEBConfig() local h=_I("Frame") h.Size=_U2(1,0,0,48) h.BackgroundColor3=_K.Card h.BorderSizePixel=0 h.LayoutOrder=0 h.Parent=_CH Corner(h,9) Stroke(h,_K.StrokeLight,1)
local t=_I("TextLabel") t.BackgroundTransparency=1 t.Position=_UO(12,6) t.Size=_U2(1,-24,0,22) t.Text="⚙️ MODO DE EXECUÇÃO" t.TextColor3=_K.White t.TextSize=14 t.Font=_GB t.TextXAlignment=_XL t.Parent=h
local st=_I("TextLabel") st.BackgroundTransparency=1 st.Position=_UO(13,28) st.Size=_U2(1,-26,0,14) st.Text="Vale pra rotas de Torre e de Parkour" st.TextColor3=_K.DarkGray st.TextSize=9 st.Font=_GM st.TextXAlignment=_XL st.Parent=h
local function opt(order,titulo,desc,valor) local c=_I("Frame") c.Size=_U2(1,0,0,66) c.BackgroundColor3=PlaybackMode==valor and _K.SelectionRedDark or _K.Card
c.BorderSizePixel=0 c.LayoutOrder=order c.Parent=_CH Corner(c,9) Stroke(c,PlaybackMode==valor and _K.SelectionRed or _K.Stroke,PlaybackMode==valor and 1.6 or 1)
local tt=_I("TextLabel") tt.BackgroundTransparency=1 tt.Position=_UO(13,9) tt.Size=_U2(1,-90,0,20) tt.Text=titulo tt.TextColor3=PlaybackMode==valor and _K.SelectionRed or _K.White
tt.TextSize=13 tt.Font=_GB tt.TextXAlignment=_XL tt.Parent=c local dd=_I("TextLabel") dd.BackgroundTransparency=1 dd.Position=_UO(13,30) dd.Size=_U2(1,-90,0,30)
dd.Text=desc dd.TextColor3=_K.Gray dd.TextSize=9 dd.Font=_GM dd.TextWrapped=true dd.TextXAlignment=_XL dd.TextYAlignment=Enum.TextYAlignment.Top dd.Parent=c
local check=_I("TextLabel") check.AnchorPoint=_V2(1,.5) check.Position=_U2(1,-12,.5,0) check.Size=_UO(26,26) check.BackgroundTransparency=1
check.Text=PlaybackMode==valor and "✓" or "" check.TextColor3=_K.SelectionRed check.TextSize=18 check.Font=_GB check.Parent=c
local btn=_I("TextButton") btn.Size=_U2(1,0,1,0) btn.BackgroundTransparency=1 btn.Text="" btn.AutoButtonColor=false btn.Parent=c
btn.MouseButton1Click:Connect(function() PlaybackMode=valor Notify("MODO",titulo.." selecionado.","Success") ShowEBDelta() end) end
opt(1,"🎯 DUMMY","Reprodução rápida e precisa: o personagem acompanha a rota exatamente como foi gravada.","Dummy")
opt(2,"🚶 IR DIRETO","O personagem anda normalmente até cada ponto da rota, com a animação padrão do Roblox.","Direto")
Content.CanvasPosition=_V2() end
ShowEBDelta=function() CurrentPage="EBDelta" ClearContent() local hb=_I("Frame") hb.Size=_U2(1,0,0,38) hb.BackgroundTransparency=1 hb.LayoutOrder=-1000 hb.Parent=_CH
local hl=_I("UIListLayout") hl.FillDirection=Enum.FillDirection.Horizontal hl.Padding=_UD(0,6) hl.Parent=hb
local bTorres=EBHotbarBtn("🏰 TORRES","Torres") bTorres.Parent=hb local bParkour=EBHotbarBtn("🏃 PARKOUR","Parkour") bParkour.Parent=hb
local bAuto=EBHotbarBtn("🤸 AUTOMAÇÃO","Automacao") bAuto.Parent=hb local bConfig=EBHotbarBtn("⚙️ CONFIG","Config") bConfig.Parent=hb
bTorres.MouseButton1Click:Connect(function() EBSubTab="Torres" ShowEBDelta() end) bParkour.MouseButton1Click:Connect(function() EBSubTab="Parkour" ShowEBDelta() end)
bAuto.MouseButton1Click:Connect(function() EBSubTab="Automacao" ShowEBDelta() end) bConfig.MouseButton1Click:Connect(function() EBSubTab="Config" ShowEBDelta() end)
if EBSubTab=="Torres" then ShowTowers() elseif EBSubTab=="Automacao" then ShowJJs() elseif EBSubTab=="Config" then ShowEBConfig() else ShowParkours() end end
...
Close.MouseButton1Click:Connect(CloseMenu) do
...
return b end ShowJJs=function() cabecalho("🤸 AUTOMAÇÃO • JJs","Clica sozinho nas bolhas dos JJs")
...
task.defer(function() local loaded=0
